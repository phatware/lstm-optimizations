...
neurons_list = [16, 32, 64, 100, 128, 200, 256]
...
for layers in range(1,7):
    for neurons in neurons_list:
        filename = "progress-"
        cmd = [args.path + "/lstm-net/recurrent-neural-net/net", args.path + "/lstm-net/bbc/all.txt", "-it", str(args.it), "-L", str(layers), "-N", str(neurons)]
        if args.mt:
            cmd.append("-mt")
            filename += "mt.csv"
        else:
